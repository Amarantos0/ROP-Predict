...
def test(TModel, tf_loader):
    ...
    for x, y in tf_loader:
        with torch.no_grad():
            ...
            y = y.permute([1, 0, 2])
            key_padding_mask = torch.zeros(enc_inputs.shape[1], enc_inputs.shape[0], dtype=torch.float32)
            # mask_pad = torch.zeros(enc_inputs.shape[1], pre_len, dtype=torch.float32)  # [batch,seq]
            #
            # key_padding_mask[:, seq_len - pre_len:] = mask_pad
            # key_padding_mask = key_padding_mask.bool().masked_fill(key_padding_mask == 0, True).masked_fill(
            #     key_padding_mask == 1, False)
    ...
    # true[true == 0] = 0.01
    # acc = 1 - (np.abs(pre - true) / true).mean()
...
model.load_state_dict(
    torch.load(
        '/home/user2/LiuProject/MYH/reaction_parameter/model/volve/no_mask/new_original_volve_well_10_14.pkl',
...
def initiate():
    ...
    for epoch in range(5000):
        ...
        if epoch % 10 == 0:
            torch.save(model.state_dict(), '/home/user2/LiuProject/MYH/reaction_parameter/model/volve/no_mask/add_new_original_volve_well_10_14.pkl')
            ...
            csv_rel_pre.to_csv('/home/user2/LiuProject/MYH/reaction_parameter/result/add/no_mask_add_well_10_14_4.csv', sep=",", index=True)
            ...
            csv_train_loss_acc_mse_mae.to_csv(
                '/home/user2/LiuProject/MYH/reaction_parameter/result/add/loss_acc_mse_mae_add_well_10_14_4.csv', sep=",", index=True)

        if (epoch + 1) % 10 == 0:
            train_loss_acc_plot(train_loss_size, 'Loss', 'loss',
                                '/home/user2/LiuProject/MYH/reaction_parameter/result/add/train_loss.png')
            train_loss_acc_plot(train_acc_size, 'Acc', 'acc',
                                '/home/user2/LiuProject/MYH/reaction_parameter/result/add/train_acc.png')
            rel_pre_plot(train_de, train_t, train_p, 'm/hr', 'train',
                         '/home/user2/LiuProject/MYH/reaction_parameter/result/add/train_rel_pre.png')
